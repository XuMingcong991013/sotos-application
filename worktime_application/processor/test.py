...
try:
    ...
    print(f"解析模型返回失败，模型未返回合法JSON数据。错误：{e}")
...
try:
    ...
    print(f"解析模型返回, 源数据表中的工时明细数据：{response_2}")
except Exception as e:
    print(f"解析模型返回失败，模型未返回合法JSON数据。错误：{e}")
...
print(f"工时分组数据统计：{grouped_worktime_data}")
...
try:
    ...
    print(f"源数据表中的工时汇总数据：{response_3}")
except Exception as e:
    print(f"解析模型返回失败，模型未返回合法JSON数据。错误：{e}")


# 5.将原表中的汇总工时数据与统计的工时数据作比较，判断是否一致，一致是True，不一致是False
print("比较原表中的汇总工时数据与统计的工时数据...")
print(f"- 统计的工时数据：{grouped_worktime_data}")
print(f"- 原表中的工时汇总数据：{response_3}")
if grouped_worktime_data == response_3:
    compare_result = True
else:
    compare_result = False
print(f"原表中的工时汇总数据与统计的工时数据的比较结果：{compare_result}")

# 错误人员明细：
error_employee_list = []
if not compare_result:
    error_employee_list.append(employee_name)
print(f"错误人员明细：{error_employee_list}")