...
当前模板标签包括 `SOTOS` 和 `优族`。SOTOS 支持“有照片占位”和“无照片”两种模式；优族复用 SOTOS 无照片正文布局，但移除全部页眉。后续会增加其他模板；文档解析阶段对所有模板共用，信息提取和 Word 生成阶段允许按模板标签扩展。
...
输入是结构化 JSON。当前使用 `templates/standard_resume.docx` 生成 SOTOS 标准简历；优族复用同一模板和正文生成逻辑，但固定使用无照片布局并移除页眉。成功返回：
...
- 测试至少覆盖：路径拼接、后缀分流、`.doc` 提前拒绝、扫描 PDF 空文本层、DOCX 参考文本、图片无参考文本、错误日志追加、警告不阻断、证据不匹配清空、专业技能原文保留、无独立项目栏目时的产品/系统型项目提取、教育日期标准化、工作年限、JSON/Excel、批量文件发现与占用处理、GUI 后台进度与安全停止、源码/EXE 资源路径、模板分发、SOTOS 与优族标签分发、优族页眉完整移除、Word 占位、补充清单、Markdown 转 Word、红色“待补充”、真实单倍行距、三行照片框完整边界，以及多段工作经历分页不误移且标题不孤行。
- 涉及 Word 排版的修改，除结构测试外还应实际生成 DOCX，并渲染成页面图片检查全部页面；确认无重叠、截断、异常空行、Markdown 残留或分页问题。
- 当前完整离线测试基线为 55 项；新增功能后数量可以增加，不能以固定数量替代 `unittest discover` 全量通过。
...
- Word 生成当前支持 SOTOS 和优族；优族仅是 SOTOS 无照片正文的无页眉变体，其他独立版式标签尚未注册。
